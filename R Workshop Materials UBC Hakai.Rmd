---
title: "R Workshop for the Pelagic Ecosystems Lab and Hakai Affiliates at UBC"
author: "Data Carpentry Contributors, Brett Johnson"
date: "2019-01-21"
...
* Download Slack
* Update or install R
* Update of install R-Studio
* Update or instal pakcages
...
## Sending Tidy Data to Hakai

Download from slack teh hakai_dfo_gsi file
...
Show photo of Tidyverse Analysis Workflow
...
Say we want to look at the condition of sockeye in the Discovery Islands and how that varies by genetic stock.
...
# Looks like we caught the most fish from D09 so let's just use that site
fish_d09 <- fish %>% 
  filter(site_id == "D09") %>% 
...
# Left joins retain all the values on the left
left_join(fish_d09,tidy_gsi, by = 'hakai_id') 

# Right joins retain all the values on the right
right_join(fish_d09,tidy_gsi)

# Full joins retain all the values on both sides
full_join(fish_d09,tidy_gsi)

# Inner joins retain only values that are in common in both 
inner_join(fish_d09,tidy_gsi)

# Anti joins shows you want doesn't have a match
anti_join(fish_d09,tidy_gsi)

so_gsi <- inner_join(fish_d09, tidy_gsi)

```

Ok let's say that we want to look at how the condition of sockeye varies by stock

```{r}
# Exploratory data visualization

so_gsi$stock_1 <- so_gsi$stock_1 %>% 
  fct_infreq() %>% 
  fct_rev()

so_gsi %>% 
  count(stock_1) %>% 
  ggplot(aes(x = stock_1, y = n)) +
           geom_point() + coord_flip()
```

### Create your own visulization

We have chlorphyll data, stock ID data, fish data.

Choose your own adventure.

https://ggplot2.tidyverse.org/reference/

Cookbook for R for formatting
